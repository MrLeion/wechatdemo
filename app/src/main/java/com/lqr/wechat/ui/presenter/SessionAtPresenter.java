    ...
    public void setAdapter() {
        if (mAdapter == null) {
            ...
            mAdapter.setOnItemClickListener((helper, parent, itemView, position) -> {
                ...
                if (content instanceof ImageMessage) {
                    ...
                    if (MediaFileUtils.isVideoFileType(fileMessage.getName())) {
                        helper.getView(com.lqr.wechat.R.id.bivPic).setOnClickListener(v -> {
                            ...
                            if (isSend) {
                                ...
                                if (receivedStatus.isDownload() || receivedStatus.isRetrieved()) {
                                    if (fileMessage.getLocalPath() != null) {
                                        ...
                                        UIUtils.showToast(UIUtils.getString(com.lqr.wechat.R.string.file_out_of_date));
                                    }
                                } else {
                                    downloadMediaMessage(message);
                                }
                    ...
                    final ImageView ivAudio = helper.getView(com.lqr.wechat.R.id.ivAudio);
            ...
            mAdapter.setOnItemLongClickListener((helper, viewGroup, view, position) -> {
                View sessionMenuView = View.inflate(mContext, com.lqr.wechat.R.layout.dialog_session_menu, null);
                mSessionMenuDialog = new CustomDialog(mContext, sessionMenuView, com.lqr.wechat.R.style.MyDialog);
                TextView tvReCall = (TextView) sessionMenuView.findViewById(com.lqr.wechat.R.id.tvReCall);
                TextView tvDelete = (TextView) sessionMenuView.findViewById(com.lqr.wechat.R.id.tvDelete);
                ...
                tvReCall.setOnClickListener(v -> RongIMClient.getInstance().recallMessage(message, "", new RongIMClient.ResultCallback<RecallNotificationMessage>() {
                    ...
                    public void onSuccess(RecallNotificationMessage recallNotificationMessage) {
                        UIUtils.postTaskSafely(() -> {
                            ...
                            UIUtils.showToast(UIUtils.getString(com.lqr.wechat.R.string.recall_success));
                        });
                    }
                    ...
                    public void onError(RongIMClient.ErrorCode errorCode) {
                        UIUtils.postTaskSafely(() -> {
                            mSessionMenuDialog.dismiss();
                            mSessionMenuDialog = null;
                            UIUtils.showToast(UIUtils.getString(com.lqr.wechat.R.string.recall_fail) + ":" + errorCode.getValue());
                        });
                    }
                }));
                tvDelete.setOnClickListener(v -> RongIMClient.getInstance().deleteMessages(new int[]{message.getMessageId()}, new RongIMClient.ResultCallback<Boolean>() {
                    ...
                    public void onError(RongIMClient.ErrorCode errorCode) {
                        UIUtils.postTaskSafely(() -> {
                            mSessionMenuDialog.dismiss();
                            mSessionMenuDialog = null;
                            UIUtils.showToast(UIUtils.getString(com.lqr.wechat.R.string.delete_fail) + ":" + errorCode.getValue());
                        });
                    }
                }));
    ...
    public void sendAudioFile(Uri audioPath, int duration) {
        if (audioPath != null) {
            ...
            if (!file.exists() || file.length() == 0L) {
                LogUtils.sf(UIUtils.getString(com.lqr.wechat.R.string.send_audio_fail));
                return;
            }
