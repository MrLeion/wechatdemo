...
public class NewFriendAtPresenter extends BasePresenter<INewFriendAtView> {
    ...
    public void loadNewFriendData() {
        if (!NetUtils.isNetworkAvailable(mContext)) {
            UIUtils.showToast(UIUtils.getString(com.lqr.wechat.R.string.please_check_net));
            return;
        }
        ...
        ApiRetrofit.getInstance().getAllUserRelationship()
                ...
                .subscribe(userRelationshipResponse -> {
                    if (userRelationshipResponse.getCode() == 200) {
                        ...
                        Observable.error(new ServerException(UIUtils.getString(com.lqr.wechat.R.string.load_error)));
                    }
                }, this::loadError);
    }
    ...
    private void setAdapter() {
        if (mAdapter == null) {
            mAdapter = new LQRAdapterForRecyclerView<UserRelationshipResponse.ResultEntity>(mContext, mData, com.lqr.wechat.R.layout.item_new_friends) {
                @Override
                public void convert(LQRViewHolderForRecyclerView helper, UserRelationshipResponse.ResultEntity item, int position) {
                    ...
                    helper.setText(com.lqr.wechat.R.id.tvName, item.getUser().getNickname())
                            .setText(com.lqr.wechat.R.id.tvMsg, item.getMessage());

                    if (item.getStatus() == 20) {//已经是好友
                        helper.setViewVisibility(com.lqr.wechat.R.id.tvAdded, View.VISIBLE)
                                .setViewVisibility(com.lqr.wechat.R.id.tvWait, View.GONE)
                                .setViewVisibility(com.lqr.wechat.R.id.btnAck, View.GONE);
                    } else if (item.getStatus() == 11) {//别人发来的添加好友请求
                        helper.setViewVisibility(com.lqr.wechat.R.id.tvAdded, View.GONE)
                                .setViewVisibility(com.lqr.wechat.R.id.tvWait, View.GONE)
                                .setViewVisibility(com.lqr.wechat.R.id.btnAck, View.VISIBLE);
                    } else if (item.getStatus() == 10) {//我发起的添加好友请求
                        helper.setViewVisibility(com.lqr.wechat.R.id.tvAdded, View.GONE)
                                .setViewVisibility(com.lqr.wechat.R.id.tvWait, View.VISIBLE)
                                .setViewVisibility(com.lqr.wechat.R.id.btnAck, View.GONE);
                    }
                    ...
                    helper.getView(com.lqr.wechat.R.id.btnAck).setOnClickListener(v -> agreeFriends(item.getUser().getId(), helper));
        ...
        ApiRetrofit.getInstance().agreeFriends(friendId)
                ...
                .flatMap(new Func1<AgreeFriendsResponse, Observable<GetUserInfoByIdResponse>>() {
                    ...
                    public Observable<GetUserInfoByIdResponse> call(AgreeFriendsResponse agreeFriendsResponse) {
                        ...
                        return Observable.error(new ServerException(UIUtils.getString(com.lqr.wechat.R.string.agree_friend_fail)));
                    }
                })
