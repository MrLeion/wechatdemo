...
public class MyLocationActivity extends BaseActivity<IMyLocationAtView, MyLocationAtPresenter> implements IMyLocationAtView, TencentLocationListener, SensorEventListener {
    ...
    @BindView(com.lqr.wechat.R.id.rlMap)
    RelativeLayout mRlMap;
    @BindView(com.lqr.wechat.R.id.map)
    MapView mMap;
    @BindView(com.lqr.wechat.R.id.ibShowLocation)
    ImageButton mIbShowLocation;
    @BindView(com.lqr.wechat.R.id.rvPOI)
...
//        mSensorManager.registerListener(MyLocationActivity.this, mOritationSensor, SensorManager.SENSOR_DELAY_NORMAL);
    ...
    protected int provideContentViewId() {
        return com.lqr.wechat.R.layout.activity_my_location;
    }
    ...
    public void onLocationChanged(TencentLocation tencentLocation, int i, String s) {
        if (i == tencentLocation.ERROR_OK) {
            ...
            if (myLocation == null) {
                myLocation = mTencentMap.addMarker(new MarkerOptions().position(latLng).icon(BitmapDescriptorFactory.fromResource(com.lqr.wechat.R.mipmap.arm)).anchor(0.5f, 0.8f));
            }
            if (accuracy == null) {
