...
//        setAdapter();
    ...
    private void updateTotalUnreadView() {
        if (mUnreadCountTotal > 0) {
            ...
            ((MainActivity) mContext).setToolbarTitle(UIUtils.getString(com.lqr.wechat.R.string.app_name) + "(" + mUnreadCountTotal + ")");
        } else {
            ((MainActivity) mContext).getTvMessageCount().setVisibility(View.GONE);
            ((MainActivity) mContext).setToolbarTitle(UIUtils.getString(com.lqr.wechat.R.string.app_name));
        }
    }

    private void setAdapter() {
        if (mAdapter == null) {
            mAdapter = new LQRAdapterForRecyclerView<Conversation>(mContext, mData, com.lqr.wechat.R.layout.item_recent_message) {
                @Override
                public void convert(LQRViewHolderForRecyclerView helper, Conversation item, int position) {
                    if (item.getConversationType() == Conversation.ConversationType.PRIVATE) {
                        ImageView ivHeader = helper.getView(com.lqr.wechat.R.id.ivHeader);
                        ...
                        if (userInfo != null) {
                            ...
                            helper.setText(com.lqr.wechat.R.id.tvDisplayName, userInfo.getName())
                                    .setText(com.lqr.wechat.R.id.tvTime, TimeUtils.getMsgFormatTime(item.getReceivedTime()))
                                    .setViewVisibility(com.lqr.wechat.R.id.ngiv, View.GONE)
                                    .setViewVisibility(com.lqr.wechat.R.id.ivHeader, View.VISIBLE);
                        }
                    } else {
                        Groups groups = DBManager.getInstance().getGroupsById(item.getTargetId());
                        //九宫格头像
                        LQRNineGridImageView ngiv = helper.getView(com.lqr.wechat.R.id.ngiv);
                        ngiv.setAdapter(mNgivAdapter);
                        ngiv.setImagesData(DBManager.getInstance().getGroupMembers(item.getTargetId()));
                        //群昵称
                        helper.setText(com.lqr.wechat.R.id.tvDisplayName, groups == null ? "" : groups.getName())
                                .setText(com.lqr.wechat.R.id.tvTime, TimeUtils.getMsgFormatTime(item.getReceivedTime()))
                                .setViewVisibility(com.lqr.wechat.R.id.ngiv, View.VISIBLE)
                                .setViewVisibility(com.lqr.wechat.R.id.ivHeader, View.GONE);
                    }

//                    helper.setBackgroundColor(R.id.flRoot, item.isTop() ? UIUtils.getColor(R.color.gray7) : UIUtils.getColor(android.R.color.white))
                    helper.setBackgroundColor(com.lqr.wechat.R.id.flRoot, item.isTop() ? com.lqr.wechat.R.color.gray8 : android.R.color.white)
                            .setText(com.lqr.wechat.R.id.tvCount, item.getUnreadMessageCount() + "")
                            .setViewVisibility(com.lqr.wechat.R.id.tvCount, item.getUnreadMessageCount() > 0 ? View.VISIBLE : View.GONE);
                    TextView tvContent = helper.getView(com.lqr.wechat.R.id.tvContent);
                    if (!TextUtils.isEmpty(item.getDraft())) {
                        ...
                        helper.setViewVisibility(com.lqr.wechat.R.id.tvDraft, View.VISIBLE);
                        return;
                    } else {
                        helper.setViewVisibility(com.lqr.wechat.R.id.tvDraft, View.GONE);
                    }
                    ...
                    if (item.getLatestMessage() instanceof TextMessage) {
                        ...
                        tvContent.setText("[" + UIUtils.getString(com.lqr.wechat.R.string.picture) + "]");
                    } else if (item.getLatestMessage() instanceof VoiceMessage) {
                        tvContent.setText("[" + UIUtils.getString(com.lqr.wechat.R.string.voice) + "]");
                        ...
                        if (MediaFileUtils.isImageFileType(fileMessage.getName())) {
                            tvContent.setText("[" + UIUtils.getString(com.lqr.wechat.R.string.sticker) + "]");
                        } else if (MediaFileUtils.isVideoFileType(fileMessage.getName())) {
                            tvContent.setText("[" + UIUtils.getString(com.lqr.wechat.R.string.video) + "]");
                        }
                    } else if (item.getLatestMessage() instanceof LocationMessage) {
                        tvContent.setText("[" + UIUtils.getString(com.lqr.wechat.R.string.location) + "]");
                        ...
                        tvContent.setText("[" + UIUtils.getString(com.lqr.wechat.R.string.wx_red_pack) + "]" + redPacketMessage.getContent());
                    }
                }
            };
            ...
            mAdapter.setOnItemLongClickListener((helper, parent, itemView, position) -> {
                Conversation item = mData.get(position);
                View conversationMenuView = View.inflate(mContext, com.lqr.wechat.R.layout.dialog_conversation_menu, null);
                mConversationMenuDialog = new CustomDialog(mContext, conversationMenuView, com.lqr.wechat.R.style.MyDialog);
                TextView tvSetConversationToTop = (TextView) conversationMenuView.findViewById(com.lqr.wechat.R.id.tvSetConversationToTop);
                tvSetConversationToTop.setText(item.isTop() ? UIUtils.getString(com.lqr.wechat.R.string.cancel_conversation_to_top) : UIUtils.getString(com.lqr.wechat.R.string.set_conversation_to_top));
                conversationMenuView.findViewById(com.lqr.wechat.R.id.tvSetConversationToTop).setOnClickListener(v ->
                ...
                conversationMenuView.findViewById(com.lqr.wechat.R.id.tvDeleteConversation).setOnClickListener(v -> {
