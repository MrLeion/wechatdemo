...
public class GroupListAtPresenter extends BasePresenter<IGroupListAtView> {
    ...
    private void setAdapter() {
        if (mAdapter == null) {
            mAdapter = new LQRAdapterForRecyclerView<Groups>(mContext, mData, com.lqr.wechat.R.layout.item_contact) {
                @Override
                public void convert(LQRViewHolderForRecyclerView helper, Groups item, int position) {
                    LQRNineGridImageView ngvi = helper.setViewVisibility(com.lqr.wechat.R.id.ngiv, View.VISIBLE)
                            .setViewVisibility(com.lqr.wechat.R.id.ivHeader, View.GONE)
                            .setText(com.lqr.wechat.R.id.tvName, item.getName())
                            .getView(com.lqr.wechat.R.id.ngiv);
